...
def update_video_details(v_id,new_name,new_time):
    cursor.execute("UPDATE videos SET name = ?,time = ? WHERE id = ?",(new_name,new_time,v_id));
    con.commit();
    pass

def delete_video(v_id):
    cursor.execute("DELETE FROM videos WHERE id = ?",(v_id,)); # here comma require dut to tuple if we write simple then it take as single variable 
    con.commit();
    pass
